...
-- - Dữ liệu mẫu
-- - Ràng buộc (CHECK, UNIQUE, DEFAULT)
-- - CHƯƠNG 2: Procedures, Functions, Triggers, Cursors, Transactions
-- - CHƯƠNG 3: Login, User, Role, Phân quyền, Backup/Restore
-- - CHƯƠNG 4: Procedures cho Form C#
-- ==================================================================================
...
-- 9. ĐƠN HÀNG (PHÂN BỐ THEO THÁNG ĐỂ LÀM BIỂU ĐỒ)
-- Tháng 1: Thấp
...
-- Tháng 11 (Hiện tại): Cao nhất (11/11)
...
-- ==================================================================================
-- PHẦN 3: THIẾT LẬP CÁC RÀNG BUỘC (CHECK, UNIQUE, DEFAULT)
-- ==================================================================================
...
ADD CONSTRAINT UQ_TaiKhoan_TenDangNhap UNIQUE (TenDangNhap),
    CONSTRAINT DF_TaiKhoan_VaiTro DEFAULT N'KhachHang' FOR VaiTro;
GO
...
ADD CONSTRAINT CK_SanPham_GiaBan CHECK (GiaBan >= 0),
    ...
    CONSTRAINT DF_SanPham_SoLuongTon DEFAULT 0 FOR SoLuongTon;
GO

-- 4. BẢNG ĐƠN HÀNG
ALTER TABLE DonHang
ADD CONSTRAINT CK_DonHang_TongTien CHECK (TongTien >= 0),
    CONSTRAINT DF_DonHang_TrangThaiThanhToan DEFAULT N'Chưa thanh toán' FOR TrangThaiThanhToan;
GO

-- 5. BẢNG ĐÁNH GIÁ
ALTER TABLE DanhGia
ADD CONSTRAINT CK_DanhGia_Diem CHECK (Diem BETWEEN 1 AND 5),
    CONSTRAINT DF_DanhGia_DuocApprove DEFAULT 0 FOR DuocApprove;
GO
...
-- 6️⃣ THU HỒI QUYỀN (VÍ DỤ)
-- REVOKE INSERT ON DonHang FROM role_NhanVien;
-- DENY DELETE ON SanPham TO role_QuanTri;
GO

-- 7️⃣ SAO LƯU VÀ PHỤC HỒI DATABASE
...
-- Script phục hồi (chạy thủ công khi cần)
/*
USE master;
GO
RESTORE DATABASE DB_SkinFood
FROM DISK = 'D:\Backup\DB_SkinFood_20250101.bak'
...
-- 1️⃣ THÊM CỘT TÀI KHOẢN CHO BẢNG NGƯỜI DÙNG (nếu cần đăng nhập riêng)
IF COL_LENGTH('NguoiDung', 'TaiKhoan') IS NULL
BEGIN
    ALTER TABLE NguoiDung ADD TaiKhoan NVARCHAR(50) NULL;
END;

IF COL_LENGTH('NguoiDung', 'MatKhau') IS NULL
BEGIN
    ALTER TABLE NguoiDung ADD MatKhau NVARCHAR(50) NULL;
END;
GO

-- 2️⃣ PROCEDURE ĐĂNG NHẬP
...
-- 5️⃣ PROCEDURES CHO QUẢN LÝ SẢN PHẨM
...
-- 6️⃣ PROCEDURES CHO QUẢN LÝ ĐƠN HÀNG
